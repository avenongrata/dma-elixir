...
struct ip_core_dchan
{
    ...
    int length;                 /* size of incoming message */
};
                            ...
                            size_t length, loff_t * pos)
{
    ...
    char * src_dma_buffer = kmalloc(length, GFP_KERNEL);

    rx_dma_handle = dma_map_single(dma->dma_rx->dchan->device->dev,
                                   src_dma_buffer, length, DMA_FROM_DEVICE);

    txd = dmaengine_prep_slave_single(dma->dma_rx->dchan, rx_dma_handle,
                                      length, DMA_DEV_TO_MEM, flags);

    /* start DMA transaction */
    ret = transfer(dma->dma_rx, txd);
    if (IS_ERR_VALUE(ret))
    {
    ...
    ret = wait_for_transfer(dma->dma_rx);
    if (IS_ERR_VALUE(ret))
    {
        /* call cleanup function */
        return ret;
    ...
    ret = copy_to_user(buffer, src_dma_buffer, length);
    if (ret)
    {
        printk(KERN_INFO "There is an error by copy to user\n");
        /* call cleanup function */
        return -1; // ret ?
    }
                            ...
                            size_t length, loff_t * pos)
{
    ...
    enum dma_ctrl_flags flags = DMA_CTRL_ACK | DMA_PREP_INTERRUPT;
    char * dst_dma_buffer = kmalloc(length, GFP_KERNEL);

    ret = copy_from_user(dst_dma_buffer, buffer, length);
    if (ret)
    {
        printk(KERN_INFO "There is some error in copy_from_user: ret = %ld\n",
               ret);
        /* call cleanup function */
        return -1; // ret ?
    }

    tx_dma_handle = dma_map_single(dma->dma_tx->dchan->device->dev,
                                   dst_dma_buffer, length, DMA_TO_DEVICE);

    txd = dmaengine_prep_slave_single(dma->dma_tx->dchan, tx_dma_handle,
                                      length, DMA_MEM_TO_DEV, flags);

    /* start DMA transaction */
    ret = transfer(dma->dma_tx, txd);
    if (IS_ERR_VALUE(ret))
    {
        /* call cleanup function */
        return ret;
    ...
    /* wait for DMA transfer */
    ret = wait_for_transfer(dma->dma_tx);
    if (IS_ERR_VALUE(ret))
    {
        /* call cleanup function */
        return ret;
